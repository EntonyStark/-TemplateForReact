...
const buildControls = props => {
	...
	return <div className="burger-control-container">
		<p>{`${currentPrise}:`} {`${props.price > 4 ? props.price.toFixed(2) : 4} $`}</p>
		{controls.map(el => (
			<Item
		...
		<button disabled={!props.purchasable} className="order-button">{orderNow}</button>
	</div>
};
